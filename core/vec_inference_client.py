...
class VecInferenceClient:
    ...
    def _using_trial_headers(self):
        return bool(
            self.trial_receipt
            and self.trial_install_key
            and self.trial_server_id
    ...
    def _get_auth_headers(self):
        ...
        if token:
            ...
            return headers
        if self._using_trial_headers():
            headers['X-Trial-Receipt'] = str(self.trial_receipt)
            headers['X-Trial-Install-Key'] = str(self.trial_install_key)
            headers['X-Trial-Id'] = str(self.trial_server_id)
        return headers
        ...
        try:
            ...
            try:
                response = requests.post(
                    ...
                    headers=self._get_auth_headers(),  # Paid: Bearer; trial: X-Trial-* (no Authorization)
    ...
    def _start_inference(
        ...
        try:
            ...
            if status_code == 401:
                ...
                if self._using_trial_headers():
                    raise Exception(
                        "Trial authentication failed (401). Receipt may be invalid or expired — try refreshing trial status and run again."
                    )
                raise Exception("Authentication failed (401). JWT token may be invalid or expired. Please re-validate your license key.")
